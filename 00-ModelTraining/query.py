...
WITH user_level AS(
    ...
    FROM 
        `project-data.dataset-data.ga_sessions_*` ga,
        UNNEST(ga.hits) hits
        WHERE
            _TABLE_SUFFIX 
            BETWEEN 
            FORMAT_DATE("%Y%m%d",DATE_SUB(PARSE_DATE('%Y-%m-%d',  '{start_date}'),INTERVAL 68 DAY)) 
            AND 
            FORMAT_DATE("%Y%m%d",DATE_SUB(PARSE_DATE('%Y-%m-%d',  '{start_date}'),INTERVAL 8 DAY))
...
FROM 
    combined
INNER JOIN labels using(user_id)
WHERE 
    user_id is not null
"""